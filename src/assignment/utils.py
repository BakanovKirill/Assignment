...
def calculate_total_fast(event):
    ...
    products_fee_amount = product_items.aggregate(
        amount=Sum(F('product__fee__amount') * F('product__fee__currency__rate') * F('quantity')))['amount']
    event_fee_amount = product_items.filter(product__fee__isnull=True).aggregate(
        amount=Sum(F('event__fee__amount') * F('event__fee__currency__rate')* F('quantity')))['amount']
